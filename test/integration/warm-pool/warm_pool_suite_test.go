...
//variables set, alter them in that test and run it once before you run the desired tests. Some prometheus metrics are
// displayed at the end of a test run. If a test fails, something happens, that makes the prometheus metrics get out of
// sorts investigate the cause. Sometimes it happens regardless though, and when it does you can run .clear() on gauges
// .reset() on counters to clear them out. Use an image with these commands, then change image back to continue
// collecting metrics.
...
const (
	...
	iterations      = 3    // run test over a set number of iterations (2,3,4,7,8)
	...
	maxPods = 30              // max pods you want to work with for your cluster (all)
...
func getMetrics(start int64, end int64) {
	netWarmMetric := "awscni_warm_net_req_count"
